...
/**
 * Module for controlling walk speed
 */
public class WalkSpeedModule extends BaseModule {
    ...
    public boolean execute(Player player, String state) {
        switch (state.toLowerCase()) {
            case "toggle":
                float currentSpeed = player.getWalkSpeed() * 10.0f; // Convert back to user-friendly scale
                sendCurrentMessage(player, currentSpeed);
                break;
            case "default":
                player.setWalkSpeed(0.2f);
                sendValueMessage(player, 2.0f);
                break;
            default:
                try {
                    float speed = Float.parseFloat(state);
                    if (speed < 1.0f || speed > 10.0f) {
                        sendErrorMessage(player, "Walk speed must be between 1.0 and 10.0.");
                        return false;
                    } else {
                        player.setWalkSpeed(speed / 10.0f); // Bukkit walk speed is between 0.1 and 1.0
    ...
    public String getUsage() {
        return "/articulate wspeed [toggle|default|1.0-10.0]";
    }
